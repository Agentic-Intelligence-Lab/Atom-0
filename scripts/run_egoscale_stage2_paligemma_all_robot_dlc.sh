...
# Fresh Stage 2 aligned-data fine-tune initialized from the PaliGemma-VLM
# all-robot checkpoint at step 97727. Submit this script as a single-node,
# 8-GPU DLC command. It never resumes or overwrites an existing experiment.

REPO_DIR=/path/to/Atom-0
TRAIN_LOG="${STAGE2_DLC_LOG:-/path/to/atom-workspace/logs/stage2_paligemma_all_robot_dlc.log}"
mkdir -p "$(dirname "${TRAIN_LOG}")"
exec > >(tee -a "${TRAIN_LOG}") 2>&1
trap 'rc=$?; echo "FAILED rc=${rc} line=${LINENO} command=${BASH_COMMAND}"; exit "${rc}"' ERR

echo "===== Stage 2 PaliGemma all-robot DLC $(date --iso-8601=seconds) ====="
cd "${REPO_DIR}"
...
export MODEL_PARAMS_PATH=/path/to/Atom-0/checkpoints/cotrain_real_robot_fix/cotrain_real_robot_fix_from_paligemma_vlm_b200_v2/97727/params
...
export EXP_NAME=paligemma_all_robot
