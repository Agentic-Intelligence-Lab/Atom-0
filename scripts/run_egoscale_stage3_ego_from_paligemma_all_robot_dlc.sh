...
# PaliGemma-lineage aligned Stage 2 checkpoint at step 30000.
# PAI DLC must launch exactly one process on each of two 8-GPU nodes.

REPO_DIR=/path/to/Atom-0
RANK_ID="${RANK:-unknown}"
TRAIN_LOG="${STAGE3_DLC_LOG:-/path/to/atom-workspace/logs/stage3_ego_from_paligemma_all_robot_30000_dlc_rank${RANK_ID}.log}"
mkdir -p "$(dirname "${TRAIN_LOG}")"
exec > >(tee -a "${TRAIN_LOG}") 2>&1
trap 'rc=$?; echo "FAILED rc=${rc} line=${LINENO} command=${BASH_COMMAND}"; exit "${rc}"' ERR

echo "===== Stage 3 Ego PaliGemma lineage DLC $(date --iso-8601=seconds) ====="
cd "${REPO_DIR}"
...
export PARAMS_PATH="${REPO_DIR}/checkpoints/egoscale_stage2_aligned/paligemma_all_robot/30000/params"
...
export EXP_NAME=stage3_ego_from_paligemma_all_robot_30000_dlc_2x8_b512_100k_20260901_v1
