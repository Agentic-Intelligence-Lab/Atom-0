...
# Alibaba PAI DLC profile for Stage 4 Piper-only full-parameter fine-tuning.
...
# Stage 4 initializes model parameters from Stage 3 while creating a fresh
# optimizer and step counter.
: "${PARAMS_PATH:?Set PARAMS_PATH to the Stage 3 checkpoint <step>/params directory}"

# Baidu guide formal-training-1 recipe: Piper30 + Piper2, Unified80, one
# eight-GPU node.
...
if [[ "${PREFLIGHT_ONLY:-0}" != "1" ]]; then
  : "${EXP_NAME:?Set a new Stage 4 EXP_NAME}"
fi
...
echo "DLC Stage 4 Piper fine-tune: world_size=${WORLD_SIZE:-1} fsdp=${FSDP_DEVICES} global_batch=${BATCH_SIZE} steps=${NUM_TRAIN_STEPS}"
