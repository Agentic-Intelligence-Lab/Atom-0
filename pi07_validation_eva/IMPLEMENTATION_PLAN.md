...
**Architecture:** Keep all new files under `/mnt/workspace/zhengdongchen/Atom-0/pi07_validation_eva`. The Python evaluator imports the read-only OpenPI training repo, confirms config/data/checkpoint metadata, reads TFDS trajectories on CPU, uses OpenPI native transforms/policy/model restore, and writes CSV/JSON/Markdown/HTML/figures per checkpoint step.
