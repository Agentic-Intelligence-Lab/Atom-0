...
# Preserve the paper's broad robot-pretraining stage after aligned mid-training.
...
# Swapped-order ablation: robot pre-training -> alignment -> ego pre-training.
# Require a shape-compatible Stage 2 checkpoint and train the full model.
_EGOSCALE_STAGE3_EGO = dataclasses.replace(
    _EGOSCALE_STAGE1_EGO,
    name="egoscale_stage3_ego",
    weight_loader=_strict_stage_checkpoint_loader(),
)

# Stage 4 repeats the production-training-1 Piper recipe as a strict downstream
# fine-tune initialized from an explicit Stage 3 checkpoint.  Keep a dedicated
# config name so its checkpoints cannot collide with the historical Stage 3 run.
