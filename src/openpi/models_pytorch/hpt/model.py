...
_FLOW_MATCHING_HEADS = frozenset({"dit", "cross_transformer", "mlp"})
_OFFICIAL_OBS_HEADS = frozenset({"diffusion", "transformer_decoder"})
...
class HPTModel(nn.Module):
    ...
    def __init__(self, config: HPTConfig, *, device: str = "cuda"):
        ...
        self.world_head = None if self.action_only else WorldHead(cond_dim=d, dino_dim=dino_dim)
    ...
    def encode_obs(
        ...
        *,
        ...
        Images/state may include an observation-horizon time dim. Official HPT
        compresses T frames inside each stem (sinusoid PE + cross-attn → fixed tokens).
        """
        device = base_img.device
        is_ego = is_ego.to(device=device, dtype=torch.bool).reshape(-1)
        is_robot = ~is_ego

        base_hist = self._maybe_crop_horizon(self._as_image_history(base_img))
        base_feat = self._encode_image_history(base_hist)
        base_ctx = self._add_horizon_sinusoid(base_feat)
        ego_tok = self._maybe_gate(self.ego_stem(base_ctx), is_ego)
        robot_tok = self._maybe_gate(self.robot_stem(base_ctx), is_robot)

        if left_wrist is None:
            left_wrist = torch.zeros_like(base_img)
        if right_wrist is None:
            right_wrist = torch.zeros_like(base_img)
        left_hist = self._maybe_crop_horizon(self._as_image_history(left_wrist))
        right_hist = self._maybe_crop_horizon(self._as_image_history(right_wrist))
        # Keep left/right time length aligned after independent random crops.
        t_w = min(left_hist.shape[1], right_hist.shape[1])
        left_hist = left_hist[:, -t_w:]
        right_hist = right_hist[:, -t_w:]
        left_feat = self._encode_image_history(left_hist)
        right_feat = self._encode_image_history(right_hist)
        ...
        state = self._maybe_crop_horizon(state.float())
    ...
    def forward_trunk_obs(self, obs_tokens: torch.Tensor) -> torch.Tensor:
        """Official heads: obs tokens only (no action/future learnable queries)."""
        return self._run_trunk(obs_tokens)

    def forward_trunk_world(self, obs_tokens: torch.Tensor) -> torch.Tensor:
        """Future-query path for world head when action head skips trunk queries."""
        if self.action_only or self.future_tokens is None:
            raise RuntimeError("forward_trunk_world requires action_world head_mode")
        b = obs_tokens.shape[0]
        future_q = self.future_tokens.expand(b, -1, -1)
        out = self._run_trunk(torch.cat([obs_tokens, future_q], dim=1))
        return out[:, -self.config.num_future_tokens :]
    ...
    def forward_trunk(self, obs_tokens: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.future_tokens is not None:
            ...
            tokens = torch.cat([obs_tokens, action_q], dim=1)
        tokens = self._apply_pos_embed(tokens)
    ...
    def training_loss(self, sample: dict[str, Any]) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        if head_type in _OFFICIAL_OBS_HEADS:
            ...
            if not self.action_only and self.world_head is not None:
                future_features = self.forward_trunk_world(obs_tokens)
            else:
            ...
            # ---- action chunk smoothness (norm space) ----
        ...
        lam_smooth = float(loss_w.get("lambda_action_smooth", 0.0))
        if self.action_only or self.world_head is None or future_features is None:
    ...
    def sample_actions(
        ...
        *,
        ...
        if head_type == "diffusion":
            global_cond = self.forward_trunk_obs(obs_tokens).mean(dim=1)
            return self.action_head.sample(global_cond, num_steps=num_steps)
        if head_type == "transformer_decoder":
            return self.action_head(self.forward_trunk_obs(obs_tokens))
