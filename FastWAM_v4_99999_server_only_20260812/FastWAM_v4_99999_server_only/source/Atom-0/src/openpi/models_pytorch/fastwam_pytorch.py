...
def _current_image_to_hwc(img: torch.Tensor) -> torch.Tensor:
    """Convert one current image per batch from policy BCHW to FastWAM BHWC."""
    if img.ndim == 5:
        img = img[:, 0]
    if img.ndim != 4:
        raise ValueError(f"Expected batched image as BCHW or BHWC, got {tuple(img.shape)}")
    if img.shape[-1] == 3:
        return img
    if img.shape[1] == 3:
        return img.permute(0, 2, 3, 1).contiguous()
    raise ValueError(f"Expected 3 image channels, got {tuple(img.shape)}")
...
class FastWAMPytorch(nn.Module):
    ...
    def sample_actions(
        ...
        **kwargs,
        ...
        current = {k: _current_image_to_hwc(img) for k, img in images.items()}
        video = _images_to_video(
