...
def test_wam_cross_piper_ft_config() -> None:
    ...
    assert cfg.pytorch_weight_path is None  # Caller must choose a matching paper-interface checkpoint.
    assert cfg.model.skip_dit_load_from_pretrain is True
    assert cfg.model.freeze_video_expert is True
    assert cfg.lr_schedule.peak_lr == pytest.approx(1.0e-4)  # Retained source default, not a paper claim.
