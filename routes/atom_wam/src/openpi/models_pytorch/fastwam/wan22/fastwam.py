...
class FastWAM(torch.nn.Module):
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        # RLDS samples video offsets across the action horizon; the two streams
        # need not have an integer raw-frame ratio (paper: 50 actions / 9 frames).
        if action_horizon <= 0:
            raise ValueError("`sample['action']` temporal dimension must be positive.")
    ...
    def _masked_domain_mean(
        ...
        """Domain contribution to the full-batch expectation, preserving sampling rho.

        An absent domain contributes zero; domains are not independently rebalanced.
        """
        mask = domain_mask.to(device=loss_per_sample.device, dtype=loss_per_sample.dtype)
        denom = loss_per_sample.numel()
