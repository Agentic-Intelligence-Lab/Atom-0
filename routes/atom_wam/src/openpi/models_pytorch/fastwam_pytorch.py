...
class FastWAMPytorch(nn.Module):
    ...
    def observation_to_sample(
        ...
        *,
        ...
        sample: dict[str, Any] = {"video": video}
        if self.config.proprio_dim is not None:
            state = _as_torch(observation.state, device=device, dtype=dtype)
            if state.shape[-1] < self.config.proprio_dim:
                state = torch.nn.functional.pad(state, (0, self.config.proprio_dim - state.shape[-1]))
            state = state[..., : self.config.proprio_dim]
            sample["proprio"] = (
                state.unsqueeze(1).expand(-1, self.action_horizon, -1) if state.ndim == 2 else state
            )
        ...
        **kwargs,
        ...
        for i in range(bsz):
            ...
            out = self.fastwam.infer_action(
                ...
                proprio=None if proprio is None else proprio[i],
                context=ctx_i,
                context_mask=ctxm_i,
                action_mask=mask_i,
