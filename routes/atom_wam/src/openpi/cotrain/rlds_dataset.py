...
class CotrainRLDSDataset:
    ...
    # Inclusive episode_index ranges kept during *training* only (standardized RLDS path).
    # E.g. ((2965, 3264), (4186, 4685)) for piper30 task subsets.
    train_episode_index_ranges: tuple[tuple[int, int], ...] | None = None
...
class CotrainRldsDataset:
    def __init__(
        ...
        *,  # Force keyword-only arguments
        ...
        def _filter_traj_by_episode_index_ranges(dataset, dataset_cfg: CotrainRLDSDataset):
            ranges = dataset_cfg.train_episode_index_ranges
            if not ranges:
                return dataset
            import tensorflow as tf

            lows = tf.constant([lo for lo, _ in ranges], dtype=tf.int64)
            highs = tf.constant([hi for _, hi in ranges], dtype=tf.int64)
            logging.info(
                "Filtering dataset '%s' to episode_index ranges %s",
                dataset_cfg.uid,
                ranges,
            )

            def _keep_traj(traj):
                ep = traj["traj_metadata"]["episode_metadata"]["episode_index"][0]
                return tf.reduce_any((ep >= lows) & (ep <= highs))

            return dataset.filter(_keep_traj)
        ...
        def _prepare_standardized(dataset, dataset_cfg: CotrainRLDSDataset):
            ...
            if is_train:
                dataset = _filter_traj_by_episode_index_ranges(dataset, dataset_cfg)
