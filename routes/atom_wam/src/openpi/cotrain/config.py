...
# piper30 train-split subset: three task blocks (760 episodes in train; 600–699 is unseen-only).
_PIPER30_THREE_TASK_TRAIN_EPISODES = 760
_PIPER30_THREE_TASK_INDEX_RANGES = (
    (600, 699),
    (2965, 3264),
    (4186, 4685),
)
# press the green button (press_button_20260414): episodes 4186–4685 (500 total; 475 train + 25 seen_test).
_PIPER30_PRESS_BUTTON_INDEX_RANGES = ((4186, 4685),)
# stack the yellow bowl on the green bowl (stack_bowls_20260413): episodes 5505–6004 (500 total; 475 train + 25 seen_test).
_PIPER30_STACK_BOWL_INDEX_RANGES = ((5505, 6004),)
# press button + stack bowl FT subset (950 train episodes).
_PIPER30_FFT_INDEX_RANGES = (
    (4186, 4685),
    (5505, 6004),
)
# push the box into the designated area (push_box_20260609): episodes 4686–5185 (500 total; 475 train + 25 seen_test).
_PIPER30_TASK4_INDEX_RANGES = ((4686, 5185),)

# Second in-house Piper RLDS drop. Its on-host builder was audited at
# /path/to/rlds/realworld_piper_2/realworld_piper_infidata/1.0.0. Keep an override for
...
_PIPER30_THREE_TASK_DATA = CotrainDataConfig(
    rlds_data_dir=_PIPER30_ROOT,
    datasets=(
        dataclasses.replace(
            _PIPER30_DATA.datasets[0],
            weight=1.0,
            train_episode_index_ranges=_PIPER30_THREE_TASK_INDEX_RANGES,
        ),
    ),
)

_PIPER30_PRESS_BUTTON_DATA = CotrainDataConfig(
    rlds_data_dir=_PIPER30_ROOT,
    datasets=(
        dataclasses.replace(
            _PIPER30_DATA.datasets[0],
            weight=1.0,
            train_episode_index_ranges=_PIPER30_PRESS_BUTTON_INDEX_RANGES,
        ),
    ),
)

_PIPER30_FFT_DATA = CotrainDataConfig(
    rlds_data_dir=_PIPER30_ROOT,
    datasets=(
        dataclasses.replace(
            _PIPER30_DATA.datasets[0],
            weight=1.0,
            train_episode_index_ranges=_PIPER30_FFT_INDEX_RANGES,
        ),
    ),
)

_PIPER30_TASK4_DATA = CotrainDataConfig(
    rlds_data_dir=_PIPER30_ROOT,
    datasets=(
        dataclasses.replace(
            _PIPER30_DATA.datasets[0],
            weight=1.0,
            train_episode_index_ranges=_PIPER30_TASK4_INDEX_RANGES,
        ),
    ),
)
...
_UNIFIED_FASTWAM = fastwam_config.FastWAMConfig(
    action_dim=cotrain_action_space.UNIFIED_ACTION_DIM,
    proprio_dim=None,  # Paper: policy conditions on current images and language only.
    action_horizon=50,
    video_num_frames=9,
...
_WAM_CROSS_ROBOT = CotrainTrainConfig(
    ...
    assets_name="atom_wam_paper50",  # Recompute stats for the paper's 50-step targets.
...
# Piper fine-tune: explicitly select a matching paper-interface checkpoint.
# Freeze Video DiT (Wan), train Action DiT + MoT.
...
# Piper30 full train split (~4927 episodes): full FT from a piper mixture ckpt.
_HPT_PIPER30_FT_FULL = dataclasses.replace(
    _HPT_PIPER_FT_FULL,
    name="hpt_cotrain_piper30_ft_full",
    data=_PIPER30_DATA,
    batch_size=256,
    num_train_steps=20_000,
    save_interval=5_000,
    eval_interval=1_000,
    val_dataset_uids=("piper30",),
...
# Piper30 press-button + stack-bowl subset (950 train episodes): frozen-trunk FT from mixture ckpt.
_HPT_FFT = dataclasses.replace(
    _HPT_PIPER_FT,
    name="hpt_cotrain_fft",
    data=_PIPER30_FFT_DATA,
    batch_size=64,
...
# Piper30 push-the-box-into-designated-area subset (475 train episodes): frozen-trunk FT (task4).
_HPT_TASK4 = dataclasses.replace(
    _HPT_FFT,
    name="hpt_cotrain_task4",
    data=_PIPER30_TASK4_DATA,
)
