...
Usage::
  ...
  cd Atom-0
  source scripts/atom0_env.sh   # sets RLDS_DATA_DIR=/path/to/rlds
...
Or explicitly::
  ...
  RLDS_DATA_DIR=/path/to/rlds PYTHONPATH=src .venv/bin/python scripts/debug_fastwam_vae_input.py ...
"""
...
class Args:
    ...
    """RLDS root (default: env ``RLDS_DATA_DIR``, else ``/path/to/rlds``). Must be set before config import."""
...
def _configure_rlds_root(args: Args) -> Path:
    ...
    if not root.is_dir():
        raise FileNotFoundError(
            ...
            "Set RLDS_DATA_DIR or pass --rlds-data-dir (e.g. /path/to/rlds)."
        )
    return root
