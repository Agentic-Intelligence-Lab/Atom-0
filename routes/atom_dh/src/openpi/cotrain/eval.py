...
def run_eval(
    ...
    *,
    ...
    n_loaders = sum(len(loaders) for loaders in val_loaders_by_label.values())
    logger.info(
        f"[eval] begin n_loaders={n_loaders} num_val_batches={num_val_batches} "
        f"labels={list(val_loaders_by_label)}"
    )
    ...
    for label, loaders in val_loaders_by_label.items():
        ...
        for name, loader in loaders.items():
            ...
            logger.info(f"[eval] start label={label} name={name}")
            it = iter(loader)
            for bi in range(num_val_batches):
                try:
                    ...
                    logger.info(
                        f"[eval] stopiteration label={label} name={name} after_bi={bi} "
                        f"dt_loader={time.monotonic() - t_loader:.1f}s"
                    )
                    break
                dt_next = time.monotonic() - t_next
                if bi == 0:
                    logger.info(
                        f"[eval] first_batch label={label} name={name} dt_next={dt_next:.1f}s"
                    )
                rng = jax.random.fold_in(base_rng, bi)
                t_fwd = time.monotonic()
                out = jax.device_get(val_flow_step(rng, train_state, batch))
                dt_fwd = time.monotonic() - t_fwd
                if bi == 0:
                    logger.info(
                        f"[eval] first_forward label={label} name={name} dt_fwd={dt_fwd:.1f}s"
                    )
                fixed_vals.append(float(out["fixed"]))
                multi_vals.append(float(out["multi"]))
            logger.info(
                f"[eval] done label={label} name={name} n_batches={len(fixed_vals)} "
                f"dt_loader={time.monotonic() - t_loader:.1f}s"
            )
