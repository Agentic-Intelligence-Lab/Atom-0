...
# Eval-only skip: these two RL2 human regex loaders can stall due to the way episodes are
# filtered and then sharded across multi-host validation. Training mixture is unchanged.
_VAL_SKIP_DATASETS = frozenset({
    "egoverse_rl2_indomain",
    "egoverse_rl2_diverse",
})
...
def build_val_loaders(
    ...
    *,
    ...
    for ds in data_config.datasets:
        if ds.uid in _VAL_SKIP_DATASETS:
            logging.info("Skipping val loader for dataset='%s' (eval-only RL2 human regex)", ds.uid)
            continue
