...
def test_staged_configs_use_expected_data_and_strict_checkpoint_loader() -> None:
    ...
    assert config._EGOSCALE_STAGE3_EGO.data is config._EGOSCALE_STAGE1_EGO_DATA
    assert {
        dataset.uid for dataset in config._EGOSCALE_STAGE3_EGO.data.datasets
    } == {
        "egoverse_aria",
        "egoverse_eva",
        "egoverse_human",
    ...
    assert config._EGOSCALE_STAGE3_EGO.freeze_filter.__class__.__name__ == "Nothing"
    assert config._EGOSCALE_STAGE3_EGO.num_train_steps == 100_000
    assert config._EGOSCALE_STAGE3_EGO.lr_schedule.warmup_steps == 2_000
    assert config._EGOSCALE_STAGE3_EGO.lr_schedule.peak_lr == pytest.approx(2.5e-5)
    assert config._EGOSCALE_STAGE3_EGO.lr_schedule.decay_steps == 100_000
    assert config._EGOSCALE_STAGE3_EGO.lr_schedule.decay_lr == pytest.approx(2.5e-6)
    assert config._EGOSCALE_STAGE3_EGO.save_interval == 5_000
    assert (
        config._EGOSCALE_STAGE3_EGO.norm_stats_assets_name
        == "egoscale_stage1_ego_cartesian_clean_rl2"
    )
    assert (
        config._EGOSCALE_STAGE4_PIPER_FINETUNE.data
        is config._REAL_ONLY_UNIFIED80_ALIYUN_RECIPE.data
    )
    assert {dataset.uid for dataset in config._EGOSCALE_STAGE4_PIPER_FINETUNE.data.datasets} == {
        "piper30",
        "piper2",
    }
    assert config._EGOSCALE_STAGE4_PIPER_FINETUNE.num_train_steps == 20_000
    assert config._EGOSCALE_STAGE4_PIPER_FINETUNE.lr_schedule.warmup_steps == 1_000
    assert config._EGOSCALE_STAGE4_PIPER_FINETUNE.lr_schedule.peak_lr == pytest.approx(2.5e-5)
    assert config._EGOSCALE_STAGE4_PIPER_FINETUNE.lr_schedule.decay_steps == 30_000
    assert config._EGOSCALE_STAGE4_PIPER_FINETUNE.lr_schedule.decay_lr == pytest.approx(2.5e-6)
    assert config._EGOSCALE_STAGE4_PIPER_FINETUNE.save_interval == 5_000
    assert config._EGOSCALE_STAGE4_PIPER_FINETUNE.norm_stats_assets_name == "cotrain_real_only"
    for staged in (
