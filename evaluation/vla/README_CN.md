...
评测工程位于 `/path/to/Atom-0/evaluation/vla`，训练仓库 `/path/to/Atom-0` 只读导入。评测结果默认写入工程内的 `results/`，不会提交到 GitHub。

## 默认运行

```bash
CHECKPOINT_ROOT=/path/to/checkpoints/cotrain_real_only/<new-exp-name> \
bash /path/to/Atom-0/evaluation/vla/scripts/run_validation.sh
```
...
```bash
CHECKPOINT_ROOT=/path/to/new-exp CHECKPOINT_STEP=25000 bash /path/to/Atom-0/evaluation/vla/scripts/run_validation.sh
CHECKPOINT_ROOT=/path/to/new-exp CHECKPOINT_STEP=30000 bash /path/to/Atom-0/evaluation/vla/scripts/run_validation.sh
```

## 只做元数据或校验

```bash
CHECKPOINT_ROOT=/path/to/new-exp bash /path/to/Atom-0/evaluation/vla/scripts/run_validation.sh --metadata-only
CHECKPOINT_ROOT=/path/to/new-exp bash /path/to/Atom-0/evaluation/vla/scripts/run_validation.sh --validate-only
```
