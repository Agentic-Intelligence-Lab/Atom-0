...
```bash
mkdir -p /path/to/datasets/EgoMimic

HF_ENDPOINT=https://hf-mirror.com \
/path/to/Atom-0/.venv/bin/huggingface-cli download \
  gatech/EgoMimic \
  --repo-type dataset \
  --include groceries_human.hdf5 groceries_robot.hdf5 \
  --local-dir /path/to/datasets/EgoMimic
```
...
```bash
ls -lh \
  /path/to/datasets/EgoMimic/groceries_human.hdf5 \
  /path/to/datasets/EgoMimic/groceries_robot.hdf5
```
...
.venv/bin/python scripts/convert_egomimic_hdf5_to_rlds.py \
  --source-hdf5 /path/to/datasets/EgoMimic/groceries_human.hdf5 \
  --output-data-dir /path/to/rlds/EgoMimic_smoke_v2 \
  --max-train-episodes 2 \
  --max-validation-episodes 1

.venv/bin/python scripts/convert_egomimic_hdf5_to_rlds.py \
  --source-hdf5 /path/to/datasets/EgoMimic/groceries_robot.hdf5 \
  --output-data-dir /path/to/rlds/EgoMimic_smoke_v2 \
  --max-train-episodes 2 \
  --max-validation-episodes 1
```
...
```text
/path/to/rlds/EgoMimic_smoke_v2/
└── ego_mimic_rlds/
...
export ATOM_EGOMIMIC_RLDS_ROOT=/path/to/rlds/EgoMimic_smoke_v2
export ASSETS_BASE_DIR=/path/to/assets
...
```text
/path/to/assets/egoscale_stage2_egomimic_groceries/
...
```text
/path/to/checkpoints/egoscale_stage1_ego/
...
export PARAMS_PATH=/path/to/checkpoints/egoscale_stage1_ego/stage1_ego_cartesian_clean_baidu_v2/5000/params
export ATOM_EGOMIMIC_RLDS_ROOT=/path/to/rlds/EgoMimic_smoke_v2
export ASSETS_BASE_DIR=/path/to/assets
...
- 从
  `/path/to/checkpoints/egoscale_stage1_ego/stage1_ego_cartesian_clean_baidu_v2/10000/params`
  严格恢复 Stage 1 参数；
...
.venv/bin/python scripts/convert_egomimic_hdf5_to_rlds.py \
  --source-hdf5 /path/to/datasets/EgoMimic/groceries_human.hdf5 \
  --output-data-dir /path/to/rlds/EgoMimic_full
...
```bash
export ATOM_EGOMIMIC_RLDS_ROOT=/path/to/rlds/EgoMimic_full
export ASSETS_BASE_DIR=/path/to/assets
...
do
  .venv/bin/python scripts/convert_egomimic_hdf5_to_rlds.py \
    --source-hdf5 "/path/to/datasets/EgoMimic/${source}" \
    --output-data-dir /path/to/rlds/EgoMimic_full
done
```
...
export PARAMS_PATH=/path/to/checkpoints/egoscale_stage1_ego/<stage1-exp>/<completed-step>/params
export ATOM_EGOMIMIC_RLDS_ROOT=/path/to/rlds/EgoMimic_full
export ASSETS_BASE_DIR=/path/to/assets
export CHECKPOINT_BASE_DIR=/path/to/checkpoints
...
export WANDB_API_KEY_FILE=/path/to/secrets/wandb_api_key
export OVERWRITE=0
export RESUME=0
