...
当前论文路线及命令见 [Atom-CL 发布指南](../routes/atom_cl/README.md)。下文保留了
历史可选配置和云端操作记录，其中 `aligned_parallel_gripper` 是旧的 absolute EEF
接口。当前 `egoscale_stage2_aligned` 使用四个自采 builder 的 2.0.0 版本：动作是
当前 canonical EEF 坐标系下的相对平移、rotation vector 和 absolute gripper closure，
预测一秒内的 50 步轨迹。请以该配置、转换器和对应 normalization metadata 为准，
不要将下文旧 14D absolute 接口直接用于当前自采 alignment 阶段。
